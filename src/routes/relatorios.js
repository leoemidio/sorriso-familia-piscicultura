...
router.get('/estoque', async (req, res) => {
  ...
  if (!['tanque', 'lote'].includes(por)) {
    return res.status(400).json({ error: 'Use ?por=tanque ou ?por=lote' });
  }
  try {
...
router.get('/consumo-racao', async (req, res) => {
  ...
  try {
    ...
    res.json(
      Object.entries(mapa).map(([data, quantidade]) => ({
        data,
        quantidade
      }))
    );
  } catch {
    res.status(500).json({ error: 'Erro no relatório de consumo de ração.' });
  }
});
...
router.get('/taxa-mortalidade', async (req, res) => {
  ...
  try {
    // soma total de peixes mortos no período
    ...
    // conta quantos lotes iniciaram no período
    const entr = await prisma.lote.count({
    ...
    res.status(500).json({ error: 'Erro no relatório de taxa de mortalidade.' });
  }
});
...
router.get('/producao-abate', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro no relatório de produção de abate.' });
  }
});
...
router.get('/faturamento', async (req, res) => {
  ...
  try {
    ...
    const mapa = {};
    vendas.forEach(v => {
      const dia = v.data.toISOString().slice(0, 10);
      mapa[dia] = (mapa[dia] || 0) + v.quantidade * v.precoUnit;
    });
    res.json(
      Object.entries(mapa).map(([data, total]) => ({
        data,
